...
public class WixChecker {
	...
	private String infoByMsisdn() {
		...
		URL url = null;
		try {
			url = new URL("http://10.254.0.50:81/complainsBSCSprod/search.jsp?id=" + mSISDN + "&msisdn=1&telnum="
					+ mSISDN);
		...
		try {
			...
			while ((line = rd.readLine()) != null) {
				if (line.startsWith(
						"      <td><input id=\"fioOrg\" class=\"required\" name=\"fioOrg\" type=\"text\"  value=\"")) {
					result.append(line.substring((line.indexOf("text\"  value=") + 14),
							line.indexOf("\" style=\"width:350px\"  /></td>"))).append(";");
				}
				if (line.contains("<input id=\"clubid\" name=\"clubid\"")) {
					result.append(line.substring(
							(line.indexOf("\"clubid\"  readonly type=\"text\" size=\"15\"  value=\"") + 49),
							line.indexOf("\"  style=\" background-color:"))).append(";");
				}
			}
			rd.close();
		} catch (IOException e) {
